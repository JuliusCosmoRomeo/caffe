...
class ErrorRateCalculator:
	def __init__(self, path_to_ground_truth):
		start = time.time()
		self.ground_truth = self.prepare_ground_truth_dict(path_to_ground_truth)
		print("imported ground truth in " + str(time.time()-start))

	#example call: top_n_error_rate(1, out, "../../ILSVRC2012.yaml", range(9))
	def top_n_error_rate(self, n, predictions, file_ids):
		file_number = 0
		all_correct_files = 0
		all_files = 0
		...
		for pred in predictions:
			...
			for index in top_n_indices:	
				...
				if category_Id in self.ground_truth:
					for file_id in self.ground_truth[category_Id]:
	...
	def prepare_ground_truth_dict(self, path_to_ground_truth):
